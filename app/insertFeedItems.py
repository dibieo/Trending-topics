...
def insert_feeditems(feed_id, url):
    ...
    for f in  feed.entries:
       ...
       #(MySQLdb.escape_string(f.title), MySQLdb.escape_string(f.description), MySQLdb.escape_string(f.link), MySQLdb.escape_string(f.guid), datetime.datetime.now(), datetime.datetime.now(), feed_id)
        ...
        cursor.executemany(
              """INSERT INTO feed_item (title, description, link, guid, pub_date, date, feed_id)
              VALUES (%s, %s, %s, %s, %s, %s, %s)""",
              [
              (f.title, f.description, f.link, f.guid, datetime.datetime.now(), datetime.datetime.now(), feed_id  ) #soheilTODO replace datetime.now (not sure which one) with pub_date 
              ] )
